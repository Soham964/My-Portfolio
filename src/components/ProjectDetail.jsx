...
import { staticProjects } from "../Pages/Portofolio"; // Import staticProjects
import {
    ...
    <div className="grid grid-cols-2 gap-3 md:gap-4 p-3 md:p-4 bg-[#0a0a1a] rounded-xl overflow-hidden relative">
      ...
      <div className="relative z-10 flex items-center space-x-2 md:space-x-3 bg-white/5 p-2 md:p-3 rounded-lg border border-blue-500/20 transition-all duration-300 hover:scale-105 hover:border-blue-500/50 hover:shadow-lg">
        ...
        <div className="flex-grow">
          ...
          <div className="text-[10px] md:text-xs text-gray-400">Technology</div>
        </div>
      </div>
...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    const selectedProject = staticProjects.find((p) => String(p.id) === id);
  ...
  console.log("Project data in ProjectDetails:", project);
  console.log("Project video path:", project?.Video);
      ...
      <div className="relative">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16">
          ...
          <div className="grid lg:grid-cols-2 gap-8 md:gap-16">
            ...
            <div 
              className="space-y-6 md:space-y-10 animate-slideInRight relative"
            >
              <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl group">
                <div className="absolute inset-0 bg-gradient-to-t from-[#030014] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                {project.Video ? (
                  <div className="relative w-full h-full" style={{ aspectRatio: '16 / 9', backgroundColor: '#000' }}>
                    {console.log("Attempting to render video element with path:", project.Video)}
                    <video
                      ...
                      autoPlay
                      preload="auto"
                      className="w-full h-full object-cover transform transition-transform duration-700 will-change-transform"
                      style={{ zIndex: 10 }}
                      onError={(e) => {
                        console.error("Video load error:", e);
                        console.error("Video element details:", e.target);
                        ...
                        img.className = "w-full h-full object-cover transform transition-transform duration-700 will-change-transform group-hover:scale-105";
                    ...
                    >
                      <source 
                        src={project.Video} 
                        type="video/mp4"
                        onError={() => console.error("Source element error")}
                      />
                      Your browser does not support the video tag.
                    </video>
                    {console.log("Video element rendered completely")}
                  </div>
                ) : (
                  <img
                    src={project.Img}
